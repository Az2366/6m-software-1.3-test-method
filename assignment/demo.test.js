...
const { setDatabaseCredential } = require("./q1-control-flow");
...
describe("Demo", () => {
    ...
    //Q1 Test
    it("show devuser:password for DEV environment", () => {
        const result1 = setDatabaseCredential("DEV");
        expect(result1).toBe("devuser:password");
      });
    
      it("show stageuser:password for STAGE environment", () => {
        const result2 = setDatabaseCredential("STAGE");
        expect(result2).toBe("stageuser:password");
      });
    
      it("show produser:password for PROD environment", () => {
        const result3 = setDatabaseCredential("PROD");
        expect(result3).toBe("produser:password");
      });
    
      it('show "Invalid environment" for an invalid environment', () => {
        console.log = jest.fn();
        setDatabaseCredential("INVALID");
        expect(console.log).toHaveBeenCalledWith("Invalid environment");
      });
